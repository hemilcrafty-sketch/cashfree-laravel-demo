...
class PaymentController extends Controller
{
    ...
    public function createOrder(Request $request)
    {
        ...
        try {
            ...
            if ($cfResponse && isset($cfResponse->cf_order_id)) {
                ...
                $sessionId = $cfResponse->payment_session_id;
    ...
    /**
     * Web-based Checkout (Direct Redirect)
     * This method handles creation and redirects the user directly to Cashfree.
     */
    public function redirectToCheckout(Request $request)
    {
        ...
        $amount = $request->amount;
        $phone = $this->sanitizePhone($request->customer_phone);
        $orderId = (string) time();
        ...
        try {
            $cfResponse = $this->cashfree->createOrder($orderId, $amount, $email, $phone);
            Log::info('Cashfree Create Order Response:', (array) $cfResponse);

            if ($cfResponse && isset($cfResponse->payment_session_id)) {
                // Use the intermediate view with Cashfree JS SDK (Official & Reliable Way)
                return view('cashfree-redirect', [
                    'paymentSessionId' => $cfResponse->payment_session_id
                ]);
            }

            throw new \Exception("Payment session not generated by Cashfree.");
        } catch (\Exception $e) {
    ...
    public function verifyPayment($orderId)
    {
        ...
        // Return JSON if requested, otherwise return the view
    ...
    private function updateOrderStatus(Order $order, $status, $cfOrderId = null, $paymentId = null)
    {
        ...
        if ($cfOrderId)
            $updateData['cf_order_id'] = $cfOrderId;
        if ($paymentId)
